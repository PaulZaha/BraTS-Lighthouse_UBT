...
class NpzDataset(Dataset):
    def __init__(self, filescsv, image_size=256, bbox_shift=5, data_aug=False):
    ...
    def __getitem__(self, index):
        ...
        with np.load(self.file_paths[file_index], allow_pickle=True, mmap_mode='r') as npz:
            gts = npz["gts"]
            imgs = npz["imgs"]
        ...
        if len(gts.shape) > 2: ## 3D image
            print(self.file_paths[file_index])
            print(gts.shape)
            i=slice_index
            img_i = imgs[i, :, :]
            gt_i = gts[i, :, :]
